...
function scheduleNotification() {
    ...
    const messages = [
        { title: "Good morning!", body: "The DSP for today has been ready! Let's today's study!" },
        { title: "Have a Break?", body: "It's noon now. Do you want to begin today's DSP?" },
        { title: "Good evening!", body: "Have you already finished you homework? Let's start the revision!" },
    ];

    for (let i = 0; i < scheduleTimes.length; i++) {
        const scheduleTime = scheduleTimes[i];
        const payload = { title: messages[i].title, body: messages[i].body };
